...
var Logger = class {
  ...
  error(message, context) {
    this.log(0 /* ERROR */, "ERROR", message, context);
  }
  /**
  ...
  warn(message, context) {
    this.log(1 /* WARN */, "WARN", message, context);
  }
  /**
  ...
  info(message, context) {
    this.log(2 /* INFO */, "INFO", message, context);
  }
  /**
   * Log debug messages
  ...
  debug(message, context) {
    this.log(3 /* DEBUG */, "DEBUG", message, context);
  }
  /**
...
var createLogger = (functionName) => {
  return new Logger(functionName);
};
var logger = new Logger("default");

// handlers/googleAuth.ts
var logger2 = createLogger("googleAuth");
var ssmClient = new SSMClient({});
var dynamoClient = new DynamoDBClient({});
var documentClient = DynamoDBDocumentClient.from(dynamoClient, {
  marshallOptions: {
...
function resolveUserName(googleUser) {
...
async function getJwtSecret() {
  ...
  try {
    ...
    logger2.error("Failed to retrieve JWT secret from SSM", {
  ...
  try {
    ...
    if (!GOOGLE_CLIENT_SECRET) {
      try {
        ...
        const ssmClient2 = new SSMClient2({ region: process.env.AWS_REGION || "ap-northeast-1" });
        const parameterName = process.env.GOOGLE_CLIENT_SECRET_PARAMETER_NAME || "/chainy/prod/google-client-secret";
        const command = new GetParameterCommand2({
          Name: parameterName,
          WithDecryption: true
        });
        const response = await ssmClient2.send(command);
...
async function getOrCreateUser(googleUser) {
  ...
  try {
    ...
    if (getResult.Item) {
      ...
      const needsUpdate = existingUser.email !== googleUser.email || existingUser.name !== resolveUserName(googleUser) || existingUser.picture !== googleUser.picture;
...
async function generateJwtToken(user) {
  ...
  return import_jsonwebtoken.default.sign(payload, jwtSecret, { algorithm: "HS256" });
}
